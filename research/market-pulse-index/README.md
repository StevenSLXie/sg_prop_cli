...
```bash
node research/market-pulse-index/pull-ura-snapshot.mjs
```

Build the soft matched-basket v0.5 prototype from the snapshot:

```bash
node research/market-pulse-index/build-soft-matched-pulse.mjs --core-only
```

Build all soft matched-basket series and render the main charts:

```bash
node research/market-pulse-index/build-soft-matched-pulse.mjs
node research/market-pulse-index/render-soft-matched-charts.mjs
```

Compare quarter-end results against URA/SingStat official non-landed indexes:

```bash
node research/market-pulse-index/compare-with-ura.mjs
node research/market-pulse-index/compare-soft-matched-with-ura.mjs
```

Compare the prototype against URA and SRX public monthly market movements:

```bash
node research/market-pulse-index/compare-three-indexes.mjs
```
...
node research/market-pulse-index/build-market-pulse.mjs \
  ...
  --out research/market-pulse-index/output \
  --save-snapshot
```
...
- `output/soft-matched-resale-by-size.png`
- `output/soft-matched-resale-by-lease-age.png`
- `output/soft-matched-resale-by-region.png`
- `output/soft-matched-resale-key-districts.png`
- `output/soft-matched-new-sale-by-size.png`
- `output/soft-matched-new-sale-by-region.png`
- `output/soft-matched-new-sale-key-districts.png`
...
The soft-matched chart renderer also keeps same-name `.svg` files as editable source artifacts.
...
## Soft Matched-Basket v0.6

The v0.6 prototype reduces within-cell quality drift without fitting a hedonic regression. For resale, it uses trailing historical basket weights and chooses the best available matched return for each historical atom:

- `project x size` with floor-distance kernel, weight multiplier `1.00`
- fallback universe cell median, weight multiplier `1.00`

If the same project cannot be matched within the same size band, the atom uses the fallback cell instead of project-only movement. Project matching uses `district x project x street` to avoid linking generic project names across unrelated streets. Fallback cells prefer `universe cell x size x tenure bucket`, where `freehold` and `999-year` rows are separated from shorter leasehold rows, then fall back to the same cell without tenure only when the tenure-specific cell is too sparse. Floor range is used only inside same-project matches; broad fallback cells use medians because applying a floor kernel across unrelated projects can turn project, age, stack, or view differences into false floor adjustments. Weights come from transactions before the current window, so the method does not use future or current-window composition as the basket. Matched and fallback returns are first reduced to normalized cell returns, then the final index return is aggregated using lagged cell weights. This final cell normalization prevents the presence of matched projects from changing the district or regional composition. A single project is capped at 15% effective weight after redistribution. Resale points with almost no matched support are shown with lower confidence. Both resale and new sale exclude multi-unit or bulk transactions where `no_of_units` is not `1`. Each output point includes matched coverage, fallback share, top project weight, floor similarity, sample size, confidence, the published-step return, and a diagnostic raw chain. The lease-age chart uses years since lease commencement parsed from `tenure`; it is a proxy because the current transaction feed does not include actual TOP/completion year, and freehold/999-year rows are not included in that chart. Public charts and `price_index` use only confidence-gated points; the raw chain is not used as a published index.

When a series is suppressed for one or more months (confidence `None`) and then resumes, `price_index` does not reset to 100 and does not splice a single-step return onto a stale level. Instead it bridges directly from the last published window to the current window, so the published level stays continuous across sample-shortage gaps without inventing a new baseline. Bridged points record `bridged_from_month`, and the return winsor band is scaled by the number of bridged steps so a legitimate multi-month move is not over-clamped.
